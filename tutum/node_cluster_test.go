...
func Test_DeployNodeCluster(t *testing.T) {
	User = "test"
	ApiKey = "test"
	...
	defer server.Close()
	url := server.URL + "/api/v1/nodecluser/" + fake_uuid_nodecluster + "/deploy/"
	...
	BaseUrl = server.URL + "/api/v1/"
	test_response, err := DeployNodeCluster(fake_uuid_nodecluster)
	if err != nil {
		t.Fatal(err)
	}
	if reflect.DeepEqual(test_response, response) != true {
...
func Test_UpdateNodeCluster(t *testing.T) {
	User = "test"
	ApiKey = "test"
	...
	BaseUrl = server.URL + "/api/v1/"
	test_response, err := UpdateNodeCluster(fake_uuid_nodecluster, []byte(`{"name": "Test-Cluster", "region": "/api/v1/region/digitalocean/lon1/", "node_type": "/api/v1/nodetype/digitalocean/1gb/", "disk": 60}`))
	if err != nil {
		t.Fatal(err)
	}
	if reflect.DeepEqual(test_response, response) != true {
...
func Test_TerminateNodeCluster(t *testing.T) {
	User = "test"
	ApiKey = "test"
	...
	BaseUrl = server.URL + "/api/v1/"
	test_response, err := TerminateNodeCluster(fake_uuid_nodecluster)
	if err != nil {
		t.Fatal(err)
	}
	if reflect.DeepEqual(test_response, response) != true {
